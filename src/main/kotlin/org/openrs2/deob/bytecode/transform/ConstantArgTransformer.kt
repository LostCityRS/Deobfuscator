...
public class ConstantArgTransformer : Transformer() {
    ...
    override fun preTransform(classes: List<ClassNode>) {
        ...
        entryPoints = UnionMemberFilter(ReflectedConstructorFilter.create(classPath))
    ...
    override fun transformCode(classes: List<ClassNode>, clazz: ClassNode, method: MethodNode): Boolean {
        val ref = MemberRef(clazz, method)
        if (inheritedMethodSets[ref] == null) {
            return false
        }

        val args = getArgs(ref)
        branchesSimplified += simplifyBranches(clazz, method, args)
        constantsPropagated += propagateConstantArgs(clazz, method, args)
        return false
    }
