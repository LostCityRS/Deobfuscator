...
public class ExpressionSorterTransformer extends Transformer {
    ...
    private static boolean isCommutative(int opcode) {
        if (opcode == Opcodes.IADD || opcode == Opcodes.LADD || opcode == Opcodes.FADD || opcode == Opcodes.DADD)
            return true;
    ...
    private static void place(MethodNode method, Expression expression) {
        switch (expression) {
            ...
            case Expression.FieldInstruction(
                    int opcode, String owner, String name, String desc, Expression argument
            ) -> {
                place(method, argument);
                method.visitFieldInsn(opcode, owner, name, desc);
            }
    ...
    public sealed interface Expression {
        ...
        record FieldInstruction(int opcode, String owner, String name, String desc,
